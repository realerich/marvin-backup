...
class MemoryRDS:
    ...
    def search_memories(self, keyword=None, category=None, memory_type=None, 
        ...
        if keyword:
            conditions.append("(content ILIKE %s OR keywords::text ILIKE %s)")
    ...
    def get_stats(self):
        ...
        with self.rds.get_connection() as conn:
            with conn.cursor() as cursor:
                ...
                # 今日新增
                cursor.execute("SELECT COUNT(*) as today FROM memories WHERE DATE(created_at) = CURRENT_DATE")
    ...
    def format_memory(self, memory):
        """格式化单条记忆"""
        # memory tuple: id, session_key, memory_type, category, content, keywords, importance_score, source, created_at, last_accessed, access_count
        msg = f"📝 #{memory[0]} [{memory[3]}]\n"
        msg += f"   {memory[4][:100]}...\n"
        msg += f"   ⭐ {memory[6]}  👁️ {memory[10]}  📅 {memory[8]}\n"
        return msg
